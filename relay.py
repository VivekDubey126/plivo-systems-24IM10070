...
def main():
    ...
    up_in = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    up_in.bind(common.SEND_TO_RELAY)
    dn_in = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    lanes = {
        up_in: (common.RELAY_TO_RECV, Impair(prof, random.Random(args.seed)), "up"),
        dn_in: (common.RELAY_TO_SEND, Impair(prof, random.Random(args.seed + 1)), "down"),
    }
